...
import SettingsModal from "./static-components/SettingsModal";
					...
					<Suspense fallback="">
						<ErrorBoundary>
							{this.state.DynamicWidgetComponents.map((struct) => {
								...
								return (
									<div key={"DynamicComp_" + struct.getPair('dbID')}>
										<SettingsModal struct={struct} refreshWithRequest={this.refreshWithRequest} />
										<div key={struct.getPair("dbID")}>
											<div className="card border-light mb-5">
												<div className="card-title text-right">
													<button className="btn" data-toggle="modal" data-target={"#SettingsModal_" + struct.getPair('dbID')}>
